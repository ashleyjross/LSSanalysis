...
outdir = '/global/cscratch1/sd/ajross/combbricks/'
...
def LRGsel(f,south=True,ebvfac=1.,Rv=3.1):
	if ebvfac == 1. and Rv == 3.1:
		gflux = f['FLUX_G']/f['MW_TRANSMISSION_G'] 
		rflux = f['FLUX_R']/f['MW_TRANSMISSION_R']   
		zflux = f['FLUX_Z']/f['MW_TRANSMISSION_Z'] 
		...
		if Rv < 3.1:
			#linear interpolation from Schlafly 2011 table
			Rg = (3.739-3.273)*(Rv-3.1)*ebvfac+Rg
			Rr = (2.113-2.176)*(Rv-3.1)*ebvfac+Rr	 
			Rz = (1.175-1.217)*(Rv-3.1)*ebvfac+Rz
			Rw1 = (-.1)*(Rv-3.1)*ebvfac+Rw1
		if Rv > 3.1:
			#linear interpolation from Schlafly 2011 table
			Rg = (3.006-3.273)*(Rv-3.1)*ebvfac+Rg
...
def ELGsel(f,south=True,ebvfac=1.,Rv=3.1):
	if ebvfac == 1. and Rv == 3.1:
		gflux = f['FLUX_G']/f['MW_TRANSMISSION_G'] 
		rflux = f['FLUX_R']/f['MW_TRANSMISSION_R']   
		zflux = f['FLUX_Z']/f['MW_TRANSMISSION_Z'] 
	...
	w = cuts.isELG_colors(gflux, rflux, zflux, south=south)
	return w
...
def putLRG(south=True):
    if south == True:
        fls = sfs
    else:
        fls = sfn
    f = fitsio.read(fls[0])
    w0 = LRGsel(f,south=south)
    fw = f[w0]
    dt = fw.dtype
    new = np.empty(len(fw),dtype=dt)
    #cols = ['BRICKID','OBJID','RA','DEC','DCHISQ','EBV','FLUX_G','FLUX_R','FLUX_Z','MW_TRANSMISSION_G','MW_TRANSMISSION_R','MW_TRANSMISSION_Z',\
    ...
    es = ''
    while s == 0:
        outf = outdir+'mysweeps/LRGdr8_'+NS+es+'.fits'
        try:
            fitsio.read(outf)
            es += 'n'
            print(es)
...
def putELG(south=True):
    if south == True:
        fls = sfs
    else:
        fls = sfn
    f = fitsio.read(fls[0])
    w0 = ELGsel(f,south=south)
    fw = f[w0]
    dt = fw.dtype
    new = np.empty(len(fw),dtype=dt)
    #cols = ['BRICKID','OBJID','RA','DEC','DCHISQ','EBV','FLUX_G','FLUX_R','FLUX_Z','MW_TRANSMISSION_G','MW_TRANSMISSION_R','MW_TRANSMISSION_Z',\
