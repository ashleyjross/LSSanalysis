...
dirpcadw = '/mnt/lustre/ashleyr/pcadw/'
dirpc = '/mnt/lustre/ashleyr/paircounts/'
...
def mksubfile_Dmurpfjack(file1,file2,jack,off=0,a=''):
	fo = open('sub'+str(jack+off)+a+'.sh','w')
	fo.write('#!/bin/bash\n')
	fo.write('#$ -V -cwd\n')
	fo.write('#PBS -q sciama1.q\n')
	for i in range(0,20):
		fo.write('~/pp2pt_Dmurp '+file1+str(i) +' '+file2+str(jack) +' -o output.'+file1+' \n')
	fo.close()
	return True

def mksuball_nran_Dmurpfjack(ranf,galf,nran,wr,njack=20,sran=0):
	fo = open('suball.sh','w')
	fo.write('#!/bin/bash\n')
	
	if sran == 0:
		for i in range(0,njack):
			mksubfile_Dmurpfjack(galf,galf,i)
			fo.write('qsub sub'+str(i)+'.sh \n')
			fo.write('sleep 1 \n')
	for j in range(sran,sran+nran):	
		off = 20+40*j
		for i in range(0,njack):
			mksubfile_Dmurpfjack(ranf+str(j)+wr,galf,i,off)
			fo.write('qsub sub'+str(i+off)+'.sh \n')
			fo.write('sleep 1 \n')
		off = 40+40*j
		for i in range(0,njack):
...
def mkran1mil(sample,NS,version,cm='',N=0,c='sci',app='.ran.fits',compmin=0):
	...
	for i in range(minc,maxc):
		#comp = f[i]['COMP']
		#if comp > compmin:
		fo.write(str(f[i]['RA'])+' '+str(f[i]['DEC'])+'\n') #rest of the information comes from matching to galaxy catalog, this gives set of randoms matching angular foot#print
		#else:
		#	no += 1
...
#from mpl_toolkits.axes_grid1.inset_locator import inset_axes
	...
	plt.savefig('foot#print'+NS+v+'.png')
	#plt.show()
	#pp.savefig()
	#pp.close()
	return True
...
def createSourcesrd_ad(file,md='deg'):
	...
	h = healpix()
	fo = open(dirpcadw+ file+'pcadw.dat','w')
...
def createSourcesrd_adJack(file,jack,NS='N2',Njack=20):
	...
	from random import random
	import gzip
	fo = open(dirpcadw+file+str(jack)+'pcadw.dat','w')
	f = open(dirpcadw+file+'pcadw.dat')
...
def ppxilcalc_LSDfjack_bs(sample,NS,version,jack,mom,zmin=.6,zmax=1.,wm='',bs=5,start=0,rmax=250,mumin=0,mumax=1.,nranf=1,njack=20,wf=False,wmu = '',rec='',rpw='',shuff=''):
...
def ppxilfile_bs(sample,NS,version,mom,zmin=.6,zmax=1.,rp='',wm='',bs=5,start=0,rmax=250,nranf=1,njack=20,mumin=0,mumax=1.,wmu='',wf=False,rec='',shuff=''):
...
def xibaoNS(sample,zmin,zmax,version='v1.8',wm='',bs=8,start=0,rmin=35,rmax=180.,md=1.,m=1.,mb='',Bp=0.4,v='n',mockn='',angfac=0,covmd='EZmock',damp='6.0',Nmock=1000,template='Challenge_matterpower',rec='_rec',covv='v4',mumin=0,mumax=1):
	#does baofits, set mb='nobao' to do no BAO fit
	from baofit_pubtest import doxi_isolike
	from Cosmo import distance
	wz = 'mz'+str(zmin)+'xz'+str(zmax)
	...
	if sample == 'ELGEZ':
		dn = np.loadtxt(dirsci+'/EZmockELGv4/xi/xi0ELGNGCEZmock'+rec+mockn+muw+bsst+'.dat').transpose()
		ds = np.loadtxt(dirsci+'/EZmockELGv4/xi/xi0ELGSGCEZmock'+rec+mockn+muw+bsst+'.dat').transpose()
		indir = ''
		outdir = dirsci+'EZmockELGv4/BAOfits/'

	if sample == 'ELGEZave':
		dn = np.loadtxt(ebossdir+'xiave0NGCELG_EZ'+rec+'angfac'+str(angfac)+bsst+'.dat').transpose()
		ds = np.loadtxt(ebossdir+'xiave0SGCELG_EZ'+rec+'angfac'+str(angfac)+bsst+'.dat').transpose()
		#if rec == 'rec':
	...
	if sample == 'QSO':
		mod = np.loadtxt('BAOtemplates/xi0iso'+template+damp+'.dat').transpose()[1]
		modsmooth = np.loadtxt('BAOtemplates/xi0smiso'+template+damp+'.dat').transpose()[1]
	if sample == 'ELG' or sample == 'ELGQPM' or sample == 'ELGEZ' or sample == 'ELGEZave':
		#if rec == '':
		...
		#if rec == '_rec':	
		#	mod = np.loadtxt('BAOtemplates/xi0Challenge_matterpower'+damp+'15.01.0.dat').transpose()[1]
		#	modsmooth = np.loadtxt('BAOtemplates/xi0smChallenge_matterpower'+damp+'15.01.0.dat').transpose()[1]
